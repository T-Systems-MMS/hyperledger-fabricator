...
jq -s '.[0].channel_group.values.OrdererAddresses.value.addresses[.[0].channel_group.values.OrdererAddresses.value.addresses|length] += "orderer'${ORDERER_NO}'.'${DOMAIN}':7050"' config.json > config1.json
