...
## Environment variables for future changes in names 
  ...
  export ORDERER_SERIES=1  # this should be empty for 1st org (Not zero 0), 1 for 2nd Org, 2 for 3rd Org and so on..
  export BASE_ORDERER=10
  export BASE_PEER=0
  echo ${Org_Name}
...
if [ "$1" == "generate-crypto" ]; then
  	...
  	if [ ! -d  fabric-orderer-ca ]; then
	 echo "Copy fabric-orderer-ca from first organization in this folder to generate the cryto material"
         exit
	fi
  	rm -rf channel-artifacts/ crypto-config/
  	
	mkdir channel-artifacts  && mkdir dynamic-containers && export FABRIC_CFG_PATH=$PWD
	...
	cp crypto-config/ordererOrganizations/$DOMAIN/orderers/orderer${BASE_ORDERER}.$DOMAIN/tls/server.crt channel-artifacts/orderer${BASE_ORDERER}.crt

	echo "The base orderer for this Org i.e. orderer${BASE_ORDERER} related certificates have been generated and exported in ./channel-artifacts/orderer${BASE_ORDERER}.srt file"
	echo "Copy orderer$BASE_ORDERER.srt in channel-artifacts of already running organization to add bootstrap base orderer"  
	echo "Cypto material generation is now complete"
fi
...
if [ "$1" == "up" ]; then
  ...
  if [ ! -f  channel-artifacts/orderer_genesis.pb ]; then
	echo "Copy orderer genesis file into channel-artifacts/orderer_genesis.pb to join network and start base orderer containers"
	 exit
  fi
  
	docker-compose up -d
	export old_orderer=$(yq e '.volumes' docker-compose-new-raft-orderer.yaml)
	sed -i -e 's/'${old_orderer::-1}'/orderer'$BASE_ORDERER'.'$DOMAIN'/g' docker-compose-new-raft-orderer.yaml
	docker-compose -f docker-compose-new-raft-orderer.yaml up -d
	cat docker-compose-new-raft-orderer.yaml> dynamic-containers/docker-compose-new-raft-orderer-$BASE_ORDERER.yaml
	echo "Containers are now up"
	sleep 50
	docker exec cli.${Org_Name,,}.$DOMAIN bash -c "scripts/update-consortium.sh $SYS_CHANNEL $Org_Name $Org_MSP $BASE_ORDERER $DOMAIN"
	sed -i -e 's/orderer'$BASE_ORDERER'.'$DOMAIN'/orderer'$((BASE_ORDERER+1))'.'$DOMAIN'/g' docker-compose-new-raft-orderer.yaml
...
if [ "$1" == "add-local-orderer" ]; then
  ...
  NEXT_SERIES=$((ORDERER_SERIES+1))
  
  if [ $ORDERER_NO -ge ${NEXT_SERIES}0 ]; then
